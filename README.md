...
Дополнительная линия (Этап 1): node-classification на **Elliptic (Bitcoin)** — линейка
...
Порядок для научного руководителя / проверяющего (по сохранённым результатам, без
повторного запуска обучения):

1. `docs/findings_summary.md` — развёрнутый защищаемый тезис: главный результат
...
Тесты запускать необязательно (полезны для engineering confidence, не для просмотра
результатов): `pytest -q`.
...
Данные не коммитятся (`data/` в `.gitignore`). Скачать с Kaggle вариант **HI-Small**:
...
> **Чекпоинты `.pt` в `.gitignore` и в репозитории отсутствуют.** Для повторного запуска
> гибрида или Streamlit-демо нужен локальный чекпоинт GNN (переобучить конфигом
> `ibm_multignn_fulldata.yaml` или перенести вручную). Какой чекпоинт нужен —
> `docs/checkpoint_transfer_note.md`.

## Результаты

Финальные числа (test, single-seed; верифицированы на Kaggle GPU):
...
* L5/L6 (Multi-PNA / Multi-PNA+EU) восстановлены из run-log: для них есть AUC-PR/F1, но
  не сохранены threshold/precision/recall/per-pattern (см. `docs/final_results_manifest.md`).
* Эмбеддинги гибрида извлекаются из чекпоинтов слабого режима ([10,10] окрестности, 2 слоя).
