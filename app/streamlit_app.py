...
def main() -> None:
    ...
    st.title("Антифрод / AML — детекция подозрительных транзакций (edge-GNN)")
    ...
    if not os.path.exists(ckpt_path):
        st.error(f"Чекпоинт не найден: {ckpt_path}. См. docs/checkpoint_transfer_note.md.")
        st.stop()
    ...
    st.caption("Демо-сэмпл намеренно содержит ВСЕ laundering-рёбра выбранного "
