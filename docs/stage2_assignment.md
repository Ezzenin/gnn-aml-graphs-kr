...
> Рекомендация плана: проще **портировать** из `github.com/IBM/Multi-GNN` (там GIN/GAT/PNA/RGCN + эти адаптации под edge-classification на IBM AML), чем писать с нуля: сверить интерфейсы, аккуратно перенести только нужное, не тянуть весь репозиторий; сохранить наш слой метрик/логирования/конфигов.
...
Выполнять по одной фазе, проверяя acceptance перед следующей. Рекомендуемые «команды задачи»:
...
Написание отчёта (разделы 4–8) идёт параллельно по мере появления таблиц/графиков.
